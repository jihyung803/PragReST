...
STAR_COUNTERFACTUAL_RATIONALIZATION_INSTRUCTION = (
    "The rationale must foreground counterfactual pragmatic reasoning.\n"
    "When possible, justify the target interpretation contrastively until you reach one clear interpretation:\n"
    "- state one more direct, stronger, or more literal alternative the speaker could have said,\n"
    "- explain what that alternative would have implied,\n"
    "- then explain why the actual utterance suggests a different intended meaning.\n"
    "Use this reasoning form explicitly when it fits the example:\n"
    "\"If the speaker had intended X, they would likely have said Y. Because they said Z instead, the intended meaning is more likely W.\"\n"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--star_rationalize_failed",
        action="store_true",
        help=(
            "For samples whose initial/reranked teacher answer fails the judge, generate a new "
            "rationale with the reference answer supplied as a hint, then save it with the original "
            "hint-free SFT prompt."
        ),
    )
    parser.add_argument(
        "--star_rationalization_rounds",
        type=int,
        default=1,
        help="Maximum rationalization attempts for failed samples when --star_rationalize_failed is set.",
    )
...
def build_star_rationalization_messages(
    task: Task,
    previous_output: str,
    feedback: str = "",
) -> list[dict[str, str]]:
    ...
    if task.task_type == "qa":
        hint_user = (
            "Your previous answer did not pass verification.\n\n"
            f"Previous final answer:\n{previous_final or '(empty)'}\n\n"
        )
        if feedback:
            hint_user += f"Verifier feedback:\n{feedback}\n\n"
        hint_user += (
            f"The correct final answer is:\n{reference}\n\n"
            "Generate a complete rationale that justifies this correct answer from the context. "
            f"{STAR_COUNTERFACTUAL_RATIONALIZATION_INSTRUCTION}"
            "Do not mention that the answer was provided as a hint. "
            "End with the exact correct final answer in \\boxed{} on the last line."
        )
    else:
        hint_user = (
            "Your previous output did not pass verification.\n\n"
            f"Previous final output:\n{previous_final or '(empty)'}\n\n"
        )
        if feedback:
            hint_user += f"Verifier feedback:\n{feedback}\n\n"
        hint_user += (
            f"The correct target output is:\n{reference}\n\n"
            "Generate a complete rationale that justifies this target output. "
            f"{STAR_COUNTERFACTUAL_RATIONALIZATION_INSTRUCTION}"
            "Do not mention that the target output was provided as a hint. "
            "End with the exact target output in \\boxed{} on the last line."
        )
...
def build_records_with_vllm_local_batched(args: argparse.Namespace, tasks: list[Task], tokenizer):
    ...
    try:
        ...
        if judge_enabled:
            ...
            star_rationalized = [False] * len(tasks)
            star_rationalization_attempts = [0] * len(tasks)
            ...
            if args.star_rationalize_failed:
                star_pending = [idx for idx in range(len(tasks)) if not final_passes[idx]]
                max_star_rounds = max(0, int(args.star_rationalization_rounds))
                for star_round in range(1, max_star_rounds + 1):
                    star_pending = [idx for idx in star_pending if not star_rationalized[idx]]
                    if not star_pending:
                        break
                    print(f"[star] rationalization_round={star_round} pending={len(star_pending)}")
                    star_messages = [
                        build_star_rationalization_messages(
                            tasks[idx],
                            assistant_texts[idx],
                            last_feedbacks[idx],
                        )
                        for idx in star_pending
                    ]
                    star_outputs = teacher_batch(star_messages)
                    for idx, rationalized in zip(star_pending, star_outputs):
                        star_rationalization_attempts[idx] = star_round
                        if has_nonempty_final_output(rationalized):
                            assistant_texts[idx] = rationalized
                            star_rationalized[idx] = True
                    empty_recovered_plain += retry_empty_with_plain(
                        {idx: messages for idx, messages in zip(star_pending, star_messages)}
                    )
                    for idx in star_pending:
                        if has_nonempty_final_output(assistant_texts[idx]):
                            star_rationalized[idx] = True
            ...
            for idx in range(len(tasks)):
                ...
                if initial_passes[idx] is False and final_passes[idx]:
                    judge_recovered += 1
                if judge_drop_failed and not final_passes[idx] and not star_rationalized[idx]:
                    keep_mask[idx] = False
                    judge_dropped += 1
                judge_info_by_idx[idx] = {
                    ...
                    "star_rationalized": bool(star_rationalized[idx]),
                    "star_rationalization_attempts": int(star_rationalization_attempts[idx]),
                    "star_hint_removed_from_sft_prompt": bool(star_rationalized[idx]),
                }
            ...
            judge_summary = {
                ...
                "star_rationalized": int(sum(1 for x in star_rationalized if x)),
...
def main() -> None:
    ...
    if args.star_rationalize_failed and not judge_requested:
        raise ValueError("--star_rationalize_failed requires judge_refine_rounds > 0 or --judge_drop_without_refine.")
    ...
    if args.gen_backend == "vllm_local_batched":
        ...
        if judge_summary is not None:
            print(
                ...
                f"recovered={judge_summary['recovered']} star_rationalized={judge_summary.get('star_rationalized', 0)} "
                f"dropped={judge_summary['dropped']}"
            )
    ...
    for task in tqdm(tasks, desc="Generating SFT targets"):
        ...
        if judge_enabled:
            ...
            last_raw_output = ""
            star_rationalized = False
            star_rationalization_attempts = 0
            ...
            if args.star_rationalize_failed and not final_pass:
                max_star_rounds = max(0, int(args.star_rationalization_rounds))
                for star_round in range(1, max_star_rounds + 1):
                    star_rationalization_attempts = star_round
                    rationalization_messages = build_star_rationalization_messages(
                        task,
                        assistant_text,
                        last_feedback,
                    )
                    assistant_text = generate_teacher(rationalization_messages)
                    if not has_nonempty_final_output(assistant_text):
                        assistant_text = generate_teacher_plain(rationalization_messages)
                        if has_nonempty_final_output(assistant_text):
                            empty_recovered_plain += 1
                    if has_nonempty_final_output(assistant_text):
                        star_rationalized = True
                        star_rationalized_count += 1
                        break

            if judge_drop_failed and not final_pass and not star_rationalized:
                judge_dropped += 1
                continue
            ...
            judge_info = {
                ...
                "judge_enable_thinking": bool(args.judge_enable_thinking),
                "star_rationalized": bool(star_rationalized),
                "star_rationalization_attempts": int(star_rationalization_attempts),
                "star_hint_removed_from_sft_prompt": bool(star_rationalized),
            }
    ...
    if judge_enabled:
        print(
            ...
            f"recovered={judge_recovered} star_rationalized={star_rationalized_count} "
            f"dropped={judge_dropped}"
        )
    if empty_recovered_plain > 0 or empty_dropped > 0:
        print(f"[teacher] empty_recovered_plain={empty_recovered_plain} empty_dropped={empty_dropped}")
